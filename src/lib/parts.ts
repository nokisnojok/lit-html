 ...
 */
export type DOMSanitizer = (value: unknown, name: string, type: 'property' | 'attribute', node: Node) => unknown;
...
export const __testOnlySetSanitizeDOMValueExperimentalMayChangeWithoutWarning = (newSanitizer: DOMSanitizer) => {
    if (sanitizeDOMValueImpl !== undefined) {
        throw new Error(
            `Attempted to overwrite existing lit-html security policy.` + ` setSanitizeDOMValue should be called at most once.`
        );
    }
    sanitizeDOMValueImpl = newSanitizer;
};

const sanitizeDOMValue: DOMSanitizer = (value: unknown, name: string, type: 'property' | 'attribute', node: Node) => {
    if (sanitizeDOMValueImpl !== undefined) {
...
export class NodePart implements Part {
    ...
    private __commitText(value: unknown): void {
        ...
        if (node === this.endNode.previousSibling && node.nodeType === 3 /* Node.TEXT_NODE */) {
            ...
            const renderedValue = sanitizeDOMValue(value, 'textContent', 'property', textNode) as string;
    ...
    private __commitTemplateResult(value: TemplateResult): void {
        ...
        if (this.value instanceof TemplateInstance && this.value.template === template) {
            ...
            if ((sanitizeDOMValueImpl !== undefined && parent.nodeName === 'STYLE') || parent.nodeName === 'SCRIPT') {
...
const getOptions = (o: AddEventListenerOptions | undefined) =>
    o && (eventOptionsSupported ? { capture: o.capture, passive: o.passive, once: o.once } : (o.capture as AddEventListenerOptions));
