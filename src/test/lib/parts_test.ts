    ...
    suite('NodePart', () => {
        ...
        suite('setValue', () => {
            ...
            test('updates a complex value to a simple one', () => {
                let value: string | TemplateResult = html`
                    <span>bar</span>
                `;
                const t = () =>
                    html`
        ...
        suite('appendIntoPart', () => {
            test('inserts part and sets values between ref node and its next sibling', () => {
                ...
                testPart.setValue(text);
                testPart.commit();
                assert.deepEqual(Array.from(container.childNodes), [startNode, testPart.startNode, text, testPart.endNode, endNode]);

                const parentText = document.createTextNode('');
                part.setValue(parentText);
                part.commit();
                assert.deepEqual(Array.from(container.childNodes), [startNode, parentText, endNode]);
            });
        });
...
suite('setSanitizeDOMValue', () => {
    ...
    setup(() => {
        __testOnlySetSanitizeDOMValueExperimentalMayChangeWithoutWarning(
            (value: unknown, name: string, type: 'property' | 'attribute' | 'text', node: Node) => {
