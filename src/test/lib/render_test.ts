...
const isTemplatePolyfilled = (HTMLTemplateElement as any).decorate != null || ((window as any).ShadyDOM && (window as any).ShadyDOM.inUse);
const testSkipForTemplatePolyfill = (test: any) => (isTemplatePolyfilled ? test.skip : test);
...
const testIfHasSymbol = (test: any) => ((window as any).Symbol === undefined ? test.skip : test);

const ua = window.navigator.userAgent;
const isIe = ua.indexOf('Trident/') > 0;

const suiteIfCustomElementsAreSupported = (suite: any) => (window.customElements != null ? suite : suite.skip);
...
suite('render()', () => {
    ...
    suite('text', () => {
        ...
        // html tag.
        test.skip('overwrites an existing (plain) TemplateInstance if one exists, ' + 'even if it has a matching Template', () => {
    ...
    suite('attributes', () => {
        ...
        test('renders attribute binding after attribute binding that moved', () => {
            render(
                html`
                    <a href="${'foo'}" class="bar"><div id=${'a'}></div></a>
                `,
                container
            );
    ...
    suiteIfCustomElementsAreSupported(suite)('custom elements', () => {
        ...
        test('uses property setters for custom elements', () => {
            ...
            render(
                html`
                    <property-tester .value=${'foo'}></property-tester>
                `,
                container
            );
