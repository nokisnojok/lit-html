...
suite('add/remove nodes from template', () => {
    ...
    test('removing nodes in template between parts renders/updates result', () => {
        const getResult = (a: any, b: any, c: any) =>
            html`
                <div name="remove"><span>remove</span></div>
                <div foo="${a}">
                    <div name="remove"><span>remove</span></div>
                    ${b}
                    <div name="remove"><span name="remove">remove</span></div>
                    <p>${c}</p>
                </div>
                <div name="remove">
                    <span name="remove"><span name="remove">remove</span></span>
                </div>
            `;
        const result = getResult('bar', 'baz', 'qux');
        const template = templateFactory(result);
    ...
    test('removing nodes in template containing parts renders/updates result', () => {
        const getResult = (a: any, b: any, c: any, r1: any, r2: any, r3: any) =>
            html`
                <div name="remove"><span>${r1}</span></div>
                <div foo="${a}">
                    <div name="remove"><span>${r2}</span></div>
                    ${b}
                    <div name="remove"><span name="remove">${r3}</span></div>
                    <p>${c}</p>
                </div>
                <div name="remove">
                    <span name="remove"><span name="remove">remove</span></span>
