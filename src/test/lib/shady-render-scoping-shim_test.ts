...
import { html } from '../../lib/shady-render';
...
suite('shady-render scoping shim', () => {
    ...
    testName = 'adopted CSS remains when rendering a TemplateResult after an ' + 'initial non-TemplateResult';
    test(testName, function() {
    ...
    testName = 'Styles inserted in the initial render through NodeParts are ' + 'scoped.';
    test(testName, function() {
        const style = document.createElement('style');
        style.innerHTML = ':host { border-top: 2px solid black; } button { font-size: 7px; }';
        const container = document.createElement('scope-3');
        document.body.appendChild(container);
        renderShadowRoot(
            html`
                ${style}<button>This is a button.</button>
            `,
            container
        );
        assert.equal(
