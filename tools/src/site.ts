...
class CustomRenderer extends marked.Renderer {
    code(code: string, language: string, _isEscaped: boolean) {
        ...
        const highlighted = prismLanguage === undefined ? code : Prism.highlight(code, prismLanguage);
        return html`
            <pre class="code language-${language}" language="${language}">
${highlighted}
</pre>
        `;
    }
    ...
    const renderPageOutline = (data: FileData) => {
        ...
        return html`
            <ul>
                ${headers.map((header) => {
                    return html`
                        <li class="page" level="${header.depth}">
                            <a href="#${getId(header.text)}">${header.text.replace('<', '&lt;')}</a>
                        </li>
                    `;
                })}
            </ul>
